...
class SimpleCNN(nn.Module):
    """
    SimpleConvolutional Neural Network with three convolutional layers followed by 
    two fully connected layers for classification. Compared to SimpleAdaptiveCNN it uses
    traditional flattening instead of Global Average Pooling, making it less flexible to different input sizes but 
    potentially more powerful and accurate for fixed-size inputs as it retains more  information.
    ie with CIFAR we go from (batch_size, 128, 4, 4) -> flattening -> (batch_size, 2048) before the fully connected layers 
    instead of (batch_size, 128) with GAP.

    REMARK: This is only suitable for square images (height = width).
    """
